...
* Password is sent **directly**.
* **Less secure**.

```text
R1 → Username + Password → R2
R2 → Verified ✓
```
...
* R2 sends a **challenge** to R1.
* R1 sends a **response** to prove its identity.
* The actual password is **not sent directly**.
...
**PAP → Password sent directly → Less secure**
