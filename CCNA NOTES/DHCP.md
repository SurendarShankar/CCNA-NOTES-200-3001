...
---
HOW DHCP & DNS WORKES RELATIVENES:

1. **DHCP provides the DNS server's IP address** to my laptop.
2. Later, when I search for **`youtube.com`**, my laptop sends a **DNS query** to that DNS server.
3. The **DNS server finds the IP address of `youtube.com`** and sends it back to my laptop.
4. My laptop then uses that IP address to connect to YouTube.
